...
export function resolveEnterpriseCapabilities(
  ...
  // Fail-closed principle: If no enterprise plan is present, fallback to STARTER (least privilege)
  if (!enterprise || !enterprise.plan) {
    const defaultStarter = PLAN_DEFINITIONS.STARTER
    return {
      tier: 'STARTER',
      name: defaultStarter.name,
      limits: { ...defaultStarter.limits },
      features: { ...defaultStarter.features },
      hasFeature: (feat: PlanFeatureKey) => Boolean(defaultStarter.features[feat]),
      isUnlimitedChannels: false,
